...
description: Why each agent needs a specific model and how to customize without breaking things
---
...
Using Sisyphus with older GPT models would be like taking your best project manager and sticking them in a room alone to debug a race condition. Wrong fit. GPT-5.4 now has a dedicated Sisyphus prompt path, but GPT is still not the default recommendation for the orchestrator.
...
Using Hephaestus with GLM or Kimi would be like assigning your most communicative, sociable developer to sit alone and do nothing but deep technical work. They'd get it done eventually, but they wouldn't shine.
...
| **GPT-5.4** | High intelligence, strategic reasoning. Default for Oracle and key fallback for Prometheus/Atlas. |
